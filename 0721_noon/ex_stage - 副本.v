...
/**********             FU instantiation                 **********/
 ...
 //input after handshake (is_data to FU_data)    
  always @( posedge clk) begin
     ...
     else if (is_valid_ns_i && ex_allin_i) begin
     //else begin
		   FU_pc           <=                   is_pc;
		   FU_alu_op       <=               is_alu_op;
...
//output with clk
    always @ (posedge clk) begin
         if (reset == `RESET_ENABLE || flush == `ENABLE) begin
                  ...
                  ex_fu_select      <=   (`Divexopway1 && `Mulexopway0)? 4'b1110 :
                                         (`Divexopway0 && `Mulexopway1)? 4'b1011 :
										 (`Divexopway1)? 4'b1100 :
										 (`Divexopway0)? 4'b0111 :
										 (`Mulexopway1)? 4'b1000 :
										 (`Mulexopway0)? 4'b0110 : 4'b0110;
...
//uncache logic
 ...
 //way0
		always @(*) begin 	//bypass to rs of way 0
			if (`RSaluopWay0 && `RDexopWay0 && (FU_scr0_addr[`DestAddr_way0] == ex_Dest_out[`DestAddr_way0])) begin
				alu_in_0[`WordDataBus_way0] = alu_result[`WordDataBus_way0];
			end//from alu0
			else if (`RSaluopWay0 && `RDexopWay1 && (FU_scr0_addr[`DestAddr_way0] == ex_Dest_out[`DestAddr_way1])) begin
 ...
 //way1
		...
		always @(*) begin//bypass to HI of way1
			...
			else if (FU_alu_op[`AluOpBus_way1] == `INSN_MFHI && ex_op[`AluOpBus_way0] == `INSN_MTHI) begin
 ...
 always @(posedge clk or `RESET_EDGE reset) begin
	...
	else if (ex_allin_i) begin //liewenhuke
		ex_valid <= is_valid_ns_i;
		end
