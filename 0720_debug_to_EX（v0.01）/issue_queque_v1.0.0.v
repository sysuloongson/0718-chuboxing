...
input ex_dst0_valid,
input ex_dst1_valid,
input [32:0] ex_dst0_data,//??EX?б┴???????????????????ив??????????бу??ив??бд?????????????
input [32:0] ex_dst1_data,
input ex_dst0_data_valid,//???б└бу?????????????б└ик???????ив????????
...
//----------- output to EX -----------//
output reg inst0_delot_flag,
output reg inst1_delot_flag,
...
output reg is_to_next_stage_valid_r,
...
reg is_to_next_stage_valid;
always@(posedge clk)begin
    is_to_next_stage_valid_r <= is_to_next_stage_valid;
...
always@(posedge clk)begin
    id_valid_ns <=id_valid_ns_r;
end


assign  inst1_pc               = is_pc                       [63:32];
assign  inst1_dst             = is_decode_info_0 [54:50];
assign  inst1_src0           = is_decode_info_0 [49:45];
assign  inst1_src1           = is_decode_info_0 [44:40];
assign  inst1_imme           = is_decode_info_0 [39:8];
assign  inst1_type           = is_decode_info_0 [7:6];
assign  inst1_meaning     = is_decode_info_0 [5:0];
assign  inst1_ptab_addr = is_ptab_addr         [9:5];
assign  inst1_exe_code   = is_exe_code           [9:5];

assign  inst0_pc               = is_pc                       [31:0];
assign  inst0_dst             = is_decode_info_1 [54:50];
assign  inst0_src0           = is_decode_info_1 [49:45];
assign  inst0_src1           = is_decode_info_1 [44:40];
assign  inst0_imme           = is_decode_info_1 [39:8];
assign  inst0_type           = is_decode_info_1 [7:6];
assign  inst0_meaning     = is_decode_info_1 [5:0];
...
always @(posedge clk)begin
	if(!rst_ | flush)begin
		...
		if((issue_queue_is_free == 2'b10)&&(id_valid_ns))begin
			...
			delot_flag     [current_item] <= id_is_delot_flag[1];
			...
			delot_flag     [current_item+1] <= id_is_delot_flag[0];
		...
		if((issue_queue_is_free == 2'b10)&&(id_valid_ns))begin
			if(current_item >= 6'h01)begin
				...
				delot_flag     [current_item-1] <= id_is_delot_flag[1];
				...
				delot_flag     [current_item] <= id_is_delot_flag[0];
		...
		if((issue_queue_is_free == 2'b10)&&(id_valid_ns))begin
			...
			delot_flag     [current_item-2] <= id_is_delot_flag[1];	
			...
			for (i=0; i<=`ITEM_NUMBER-1; i=i+1)begin 
				...
				inst_meaning[1] <= inst1_meaning;	
				data_valid    [1] <= is_decode_valid_1;
				ptab_addr      [1] <= inst0_ptab_addr;
				exe_code        [1] <= inst0_exe_code;
				delot_flag     [1] <= id_is_delot_flag[0];	
...
assign inst0_issue_enable = (!inst0_conflict_detect)? 1'b1 : 1'b0;
assign inst1_issue_enable = (!inst1_conflict_detect)? 1'b1 : 1'b0;
...
always @ (*)begin
	...
	if(item_busy[0])begin
		...
		inst0_src0_data_ready      = 1'b0;
		inst0_src1_data_ready      = 1'b0;
	end	
...
reg [4:0]inst0_src0_conflict_detect_r;
reg [4:0]inst0_src1_conflict_detect_r;
reg [4:0]inst1_src0_conflict_detect_r;
reg [4:0]inst1_src1_conflict_detect_r;
reg [4:0]inst0_src0_data_ready_r;
reg [4:0]inst0_src1_data_ready_r;
reg [4:0]inst1_src0_data_ready_r;
reg [4:0]inst1_src1_data_ready_r;

always@(posedge clk)begin
	inst0_src0_conflict_detect_r <= inst0_src0_conflict_detect;
	inst0_src1_conflict_detect_r <= inst0_src1_conflict_detect;
	inst1_src0_conflict_detect_r <= inst1_src0_conflict_detect;
	inst1_src1_conflict_detect_r <= inst1_src1_conflict_detect;
	inst0_src0_data_ready_r <= inst0_src0_data_ready;
	inst0_src1_data_ready_r <= inst0_src1_data_ready;
	inst1_src0_data_ready_r <= inst1_src0_data_ready;
	inst1_src1_data_ready_r <= inst1_src1_data_ready;
end
	...
	if((item_busy[0])&&(inst0_conflict_detect == 1'b0))begin
		...
		case (inst0_src0_conflict_detect)
			`INST0_SRC0_CONFLICT_WITH_EX_DST1:begin
				inst0_issue_enable = (inst0_src0_data_ready)? 1'b1 ???1'b0;
			end
			...
			`INST0_SRC0_CONFLICT_WITH_WB_DST1,`INST0_SRC0_CONFLICT_WITH_WB_DST0,`INST0_SRC0_CONFLICT_WITH_NOTHING:begin
				case (inst0_src1_conflict_detect)
					`INST0_SRC1_CONFLICT_WITH_EX_DST1:begin
						inst0_issue_enable = (inst0_src1_data_ready)? 1'b1 ???1'b0;
					end
		...
		if((item_busy[1])&&(inst1_conflict_detect == 1'b0))begin
			...
			case (inst1_src0_conflict_detect)
				`INST1_SRC0_CONFLICT_WITH_EX_DST1:begin
					inst1_issue_enable = (inst1_src0_data_ready)? 1'b1 ???1'b0;
				end
				...
				`INST1_SRC0_CONFLICT_WITH_WB_DST1,`INST1_SRC0_CONFLICT_WITH_WB_DST0,`INST1_SRC0_CONFLICT_WITH_NOTHING:begin
					case (inst1_src1_conflict_detect)
						`INST1_SRC1_CONFLICT_WITH_EX_DST1:begin
							inst1_issue_enable = (inst1_src1_data_ready)? 1'b1 ???1'b0;
						end
...
always @(posedge clk )begin
	...
	if(inst1_to_fu != 3'b111)begin
		...
		inst1_valid                            <= 1'b0;
		inst1_delot_flag                  <= 1'b0;
	end	
end


always @ (*)begin
	case (inst0_src0_conflict_detect_r)
		...
		`INST0_SRC0_CONFLICT_WITH_WB_DST1: regfile_data0 = (wb_dst1_data_valid)? wb_dst1_data : 32'b0;
		`INST0_SRC0_CONFLICT_WITH_WB_DST0: regfile_data0 = (wb_dst0_data_valid)? wb_dst0_data : 32'b0;
		`INST0_SRC0_CONFLICT_WITH_NOTHING: regfile_data0 = read_regfile_data0;
		default:                           regfile_data0 = read_regfile_data0;
	endcase

	case (inst0_src1_conflict_detect_r)
		...
		`INST0_SRC1_CONFLICT_WITH_WB_DST1: regfile_data1 = (wb_dst1_data_valid)? wb_dst1_data : 32'b0;
		`INST0_SRC1_CONFLICT_WITH_WB_DST0: regfile_data1 = (wb_dst0_data_valid)? wb_dst0_data : 32'b0;
		`INST0_SRC1_CONFLICT_WITH_NOTHING: regfile_data1 = read_regfile_data1;
		default:                           regfile_data1 = read_regfile_data1;
	endcase	
	
	case (inst1_src0_conflict_detect_r)
		...
		`INST1_SRC0_CONFLICT_WITH_WB_DST1: regfile_data2 = (wb_dst1_data_valid)? wb_dst1_data : 32'b0;
		`INST1_SRC0_CONFLICT_WITH_WB_DST0: regfile_data2 = (wb_dst0_data_valid)? wb_dst0_data : 32'b0;
		`INST1_SRC0_CONFLICT_WITH_NOTHING: regfile_data2 = read_regfile_data2;
		default:                           regfile_data2 = read_regfile_data2;		
	endcase
	
	case (inst1_src1_conflict_detect_r)
		...
		`INST1_SRC1_CONFLICT_WITH_WB_DST1: regfile_data3 = (wb_dst1_data_valid)? wb_dst1_data : 32'b0;
		`INST1_SRC1_CONFLICT_WITH_WB_DST0: regfile_data3 = (wb_dst0_data_valid)? wb_dst0_data : 32'b0;
